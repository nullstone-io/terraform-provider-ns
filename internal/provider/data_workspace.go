...
This is typically used to construct unique resource names. See unique_name.`,
		...
		},
		{
			...
			Type:            tftypes.String,
			Description:     "The data classification (sensitivity) level configured for this workspace, e.g. `customer-content`. Empty when unclassified.",
			DescriptionKind: tfprotov5.StringKindMarkdown,
			Computed:        true,
		},
		{
...
func (d *dataWorkspace) Read(ctx context.Context, config map[string]tftypes.Value) (map[string]tftypes.Value, []*tfprotov5.Diagnostic, error) {
	...
	return map[string]tftypes.Value{
		...
		"data_classification": tftypes.NewValue(tftypes.String, planConfig.ClassificationLevel),
